...
var AESEncryptUtil = (function () {
    function AESEncryptUtil() {
    }
    AESEncryptUtil.getUserInfo = function (loadingView, stage) {
        var search = window.location.search;
        var params = getSearchString('params', search); //结果：2
        ...
        function getSearchString(key, Url) {
            ...
            for (var i_1 = 0; i_1 < arr.length; i_1++) {
                var tmp_arr = arr[i_1].split("=");
                obj[decodeURIComponent(tmp_arr[0])] = decodeURIComponent(tmp_arr[1]);
            }
            return obj[key];
        }
        var key = CryptoJS.enc.Utf8.parse('wjyg12347890WJYG');
        var iv = CryptoJS.enc.Utf8.parse('wjygWJYG12347890');
        var custId;
        var headerImage;
        var version;
        var plat;
        ...
        var accountName;
        if (params) {
            var decryptJson = JSON.parse(CryptoJS.AES.decrypt(params, key, { iv: iv, mode: CryptoJS.mode.CBC, padding: CryptoJS.pad.ZeroPadding }).toString(CryptoJS.enc.Utf8));
        ...
        var bytes = this.stringToByte("DC0535352D6921&!:095A808D36EB8!!");
    ...
    AESEncryptUtil.getPlace = function () {
        var _this = this;
        return new Promise(function (resolve) {
            _this.httpRequestPost(null, "goods/list", function (res) {
                console.log("js********=" + res + "======" + res[0][0].length + "------" + res[0][0][0].length);
                resolve(res);
            });
        });
    };
    AESEncryptUtil.showPlace = function () {
        return __awaiter(this, void 0, void 0, function () {
            var place;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, this.getPlace()];
                    case 1:
                        place = _a.sent();
    ...
    AESEncryptUtil.httpRequestPost = function (params, url, func, that, domian, isJson, isEncry) {
        if (that === void 0) { that = null; }
        if (domian === void 0) { domian = this.isProd ? this.prod['url'] : this.test['url']; }
        if (isJson === void 0) { isJson = false; }
        if (isEncry === void 0) { isEncry = false; }
        ...
        if (isEncry) {
            ...
            request.addEventListener(egret.Event.COMPLETE, function (event) {
                console.log("---------------%%%%%%%%%%%%--------------");
                //alert("接收成功！！"+decrypt.toString());
                console.log(event.currentTarget.response.toString());
                func(event.currentTarget.response.toString());
                console.log("---------------************--------------");
            }, this);
    ...
    AESEncryptUtil.httpRequest = function (params, url, that, domian, isJson, isEncry) {
        var _this = this;
        if (that === void 0) { that = null; }
        if (domian === void 0) { domian = this.isProd ? this.prod['url'] : this.test['url']; }
        if (isJson === void 0) { isJson = false; }
        ...
        return new Promise(function (resolve, reject) {
            if (isEncry) {
                ...
                if (isJson) {
                    ...
                    req.setRequestHeader("custId", _this.encryptId);
                    console.log("898989899hnnn");
                }
                req.send(params);
                req.addEventListener(egret.Event.COMPLETE, function (event) {
                    ...
                    resolve(event.currentTarget.response.toString());
                    console.log("---------------************--------------");
                }, _this);
                req.addEventListener(egret.IOErrorEvent.IO_ERROR, function (event) {
                    console.log("post error : " + event);
    ...
    AESEncryptUtil.stringToByte = function (str) {
        var bytes = new Array();
        var len, c;
        len = str.length;
        for (var i = 0; i < len; i++) {
    ...
    AESEncryptUtil.byteToString = function (arr) {
    ...
    AESEncryptUtil.isProd = false;
    return AESEncryptUtil;
}());
__reflect(AESEncryptUtil.prototype, "AESEncryptUtil");
